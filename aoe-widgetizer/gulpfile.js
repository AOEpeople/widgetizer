...
gulp.task('widgetizer', ['clean', 'test', 'copy'], function() {
    return browserify('./src/js/WidgetizerGlobalAndAMD.js')
        ...
        .pipe(source('widgetizer_standalone.min.js'))
        .pipe(gulp.dest(paths.dist));
});
...
gulp.task('_unittest', ['start_mock_server'], function() {
...
gulp.task('build', ['widgetizer'/*, 'widgetizer_jquery'*/]);
